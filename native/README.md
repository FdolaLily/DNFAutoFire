...
- 奔跑/连招是可用虚拟时钟测试的状态机；首次确认使用用户设置的搓招保护，保留 350ms 方向序列保护、90ms 变向确认和斜轴继承。同一时刻只有一个方向执行双击，其余按住的轴保持按下，在它完成时加入；被其他方向打断的双击最多重发 3 次，之后保持行走，输入不再变化时输出也不再变化（不会无限重发）。双方向起跑时，第二方向若在首方向确认或双击期间加入，会要求首方向重新完成连续的同方向双击，再将两轴标记为奔跑；交替斜向变向会清除旧的奔跑标记，按实际发出的方向按下顺序确认双击，水平反向优先完成自身双击，恢复一个与朝向相反的水平方向也会结束旧的斜向奔跑。已确认奔跑后加入正交方向仍直接继承。Combo 触发键和奔跑方向键从普通连发候选中剔除。
- 相反方向：两个相反方向同时按住时后按的生效，先按的暂时抬起，松开后按的方向时恢复并重新双击起跑（静止与奔跑中一致）。玩家自己的快速双击（第一下按住不超过 250ms、抬起后 200ms 内再按下，中间没有其他方向或技能）直接视为已起跑，不再补发。
- 技能与奔跑：任何非方向键按下都视为结束奔跑，按住期间方向键只行走。程序托管的技能（手动连发键、已启用的旅人/战法/剑宗触发键、连招触发键及整段连招）全部松开且连招播放完后，若技能前正在奔跑（或正处于恢复双击中），立即补一次完整双击；其他键（跳跃、道具、未加入连发的技能等），或技能前并未奔跑时，松开后仍按住的方向按搓招保护重新起跑。焦点返回前就按住、Down 未被观察到的键同样在松开后放行。
- 可玩的连招触发键与已启用职业的触发键优先于同一物理键上的奔跑方向（否则奔跑 hook 会先吞掉按键）；该方向不参与奔跑，奔跑设置中提示“方向键被连招/职业占用”，设置方向键时拒绝这类按键。
- 控制器重启（游戏内开关、快速切换方案、设置生效）只在 1 秒内的立即重启交接仍按住的键：新方案的方向键重新计时起跑；旧方案中按下的方向若不再是奔跑键，则以原扫描码形式交还游戏继续行走，直到实体抬起；其余按住的键（包括开关热键本身）推迟起跑到松开。切换到安全桌面（Ctrl+Alt+Del、Win+L、UAC）时视为所有键已抬起；DNF 重新获得焦点和控制器启动时，按系统异步键态清除直通键丢失的抬起（方向键与被吞掉的键除外，它们的异步键态含注入边沿）。
- 游戏内热键由同一个 hook 识别（`hotkeyAction`）：快速切换在任何窗口生效且不吞键；连发总开关（默认 Alt+F12）只在 DNF 前台、按精确修饰键匹配，一键奔跑开关（默认 F10）只在 DNF 前台且连发运行时匹配，二者的按下与对应抬起都被吞掉。hook 只向界面线程投递 `InputCommand::TogglePower` / `ToggleRun`（LPARAM 为前台 DNF 窗口），控制器停止期间（`enabled=false`）同样运行，所以热键能重新开启连发。
...
只有普通托管键的硬件输入被抑制；职业专用输出的实体操作与方向键仍直通各自机制。从界面启动连发时已经按住的键不会被注入，从下一次硬件事件开始建立状态；游戏内开关奔跑、快速切换方案时的交接见上。
...
- 游戏内提示 `DAF.Native.Notice`：热键或托盘开关后，按 `game_window.h` 找到的 DNF 窗口客户区右下角（留 28 DIP 边距）放置一个无所有者的分层置顶窗口，`WS_EX_NOACTIVATE | WS_EX_TRANSPARENT`，以 `SW_SHOWNOACTIVATE` 显示、`WM_MOUSEACTIVATE` 返回 `MA_NOACTIVATE`、`WM_NCHITTEST` 返回 `HTTRANSPARENT`；淡入后停留 2.2 秒淡出。它不接触游戏进程，DNF 独占全屏时可能看不到。
...
- 程序与托盘图标在 `icons/`：`app.ico`（资源 1，深色键帽 + 方波字符 + 右上角绿灯）、`running.ico`（资源 2，托盘运行中，绿灯）、`stopped.ico`（资源 3，托盘未启动，红灯）。颜色取自暗黑主题色板，由 `scripts/build-icons.py` 生成，16–32px 使用单独的简化几何。
